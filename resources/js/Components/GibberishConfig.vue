    ...
    <div class="px-6 py-3 bg-white relative rounded dark:bg-slate-500">
        ...
        <div class="lg:flex lg:items-center lg:justify-between">
            <div class="flex-1 min-w-0">
                <h2 @click="goToConfigPage" class="text-2xl font-bold leading-7 text-gray-900 cursor-pointer dark:text-gray-100 sm:text-3xl sm:truncate">
                    {{ config.title }}
                </h2>
                <div>
                    <p class="mt-1 leading-5 text-gray-500 whitespace-pre dark:text-white line-clamp-1" v-html="config.description" />
                </div>
                <div class="flex flex-col mt-2 sm:flex-row sm:flex-wrap sm:space-x-6" title="category">
                    <div class="flex items-center mt-2 text-sm text-gray-500 dark:text-gray-300">
                        <img class="flex-shrink-0 mr-1.5 h-6 w-6 text-gray-400 dark:text-gray-200" :src="'/icons/' + config.category.icon_path" :alt="config.category.title">
                        {{ config.category.title }}
                    </div>
                    <div class="flex items-center mt-2 text-sm text-gray-500 dark:text-gray-300" title="author">
                        <svg xmlns="http://www.w3.org/2000/svg" class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400 dark:text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        {{ config.user.username }}
                    </div>
                    <div class="flex items-center mt-2 text-sm text-gray-500 uppercase dark:text-gray-300" title="language">
        ...
        <modal :show="showModal" @close="closeModal" max-width="md">
            <div class="p-10">
                <div class="prose break-words truncate whitespace-normal mb-14" v-html="config.formatted_description" />
